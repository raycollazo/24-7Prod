...
class ReportEmployeeHours(models.Model):
    ...
    employee_standard_rate = fields.Monetary(
    ...
    job_class_id = fields.Many2one(
        string = 'Crew Member Class',
        comodel_name = 'job.class'
    )


    @api.onchange('job_class_id')
    def check_jobclass(self):
        today = datetime.now().strftime('%Y-%m-%d')
        for record in self:
            if record.report_id.pv_site:
                #raise ValidationError([('start_date','<',today),('end_date','<',today), \
                #                                               ('job_class_id','=',record.job_class_id.id)])
                rate = self.env['job.prevailing.wage'].search(['|',('start_date','<=',today),('start_date','=',False), \
                                                               '|',('end_date','>=',today),('end_date','=',False), \
                                                               ('job_class_id','=',record.job_class_id.id)]).wage
                if not rate > 0:
                    raise ValidationError('This Daily Report cannot be entered yet!' + \
                                          "\n\nThis Site is eligible for Prevailing Wage Rates but" + \
                                          ' no valid rate for the ' + record.job_class_id.name + ' Job Class was found.' \
                                          )
    ...
    def _onchange_driver_time_depart_str(self):
        for rec in self:
            ...
            if rec.driver_time_depart > 0 and rec.driver_time_end > 0:
                rec.total_passenger_hours_after = 0
    ...
    @api.multi
    @api.depends('total_drive_hours_before','total_drive_hours_after')
    def _total_hours(rec):
        for record in rec:
            record.total_hours = record.total_regular_hours + \
                                 record.total_pw_hours + \
                                 record.total_drive_hours + \
                                 record.total_passenger_hours
        return rec
    ...
    def _pw_cost(self):
        today = datetime.now().strftime('%Y-%m-%d')
        for record in self:
            record.pw_cost = self.env['job.prevailing.wage'].search(['|',('start_date','<=',today),('start_date','=',False), \
                                                               '|',('end_date','>=',today),('end_date','=',False), \
                                                               ('job_class_id','=',record.job_class_id.id)]).wage
    ...
    def _total_pay(rec):
        for record in rec:
            ...
            record.total_pay = record.regular_pay + record.passenger_pay + record.drive_pay + record.pw_pay
...
class JobDailyReport(models.Model):
    ...
    def action_review(self):
        reviewed = True
        for line in self.employee_hours:
            if not line.reviewed:
                reviewed = False
        if not reviewed:
            raise ValidationError('This Daily Report cannot be set to Reviewed status yet!' + \
                                    "\n\nThe 'Review' box next to ALL Employee Hours must be checked." )
    ...
    def action_approve(self):
        ...
        for line in self.employee_hours:
            if not line.approved:
                approved = False
        if not approved:
            raise ValidationError('This Daily Report cannot be set to Approved status yet!' + \
                                    "\n\nThe 'Approve' box next to ALL Employee Hours must be checked." )


        new_task = self.env['project.task']
        task_id = new_task.create({
            'name': 'Daily Report: ' + self.name,
            'project_id': self.job_id.id,
            'kanban_state': 'normal',
        })
        ...
        for line in self.employee_hours:
            time_line.create({
                ...
                'task_id': task_id.id,
                'name': 'Regular Hours',
                'rate': line.timesheet_cost,
                'unit_amount': line.total_regular_hours,
            })
    ...
    pv_site = fields.Boolean(
        string = 'Prevailing Wage Site',
        related = 'site_id.pv_site',
    )

    # Check if Site is NULL
    @api.onchange('site_id')
    def site_id_onchange(self):
        if not self.site_id  and not self.job_id.site_id:
            raise ValidationError('This Daily Report cannot be entered yet!' + \
                                  '\n\nPlease return to the Job and select or enter a Site.')
    ...
    def crew_employees_change(self):
        ...
        default_cost_code_id = self.env['job.cost.code'].search([('code','=','101')]).id
        default_job_class_id = self.env['job.class'].search([('name','=','Standard')]).id
        ...
        for employee in self.crew_employees:
            if not employee.user_id:
                raise ValidationError('This Daily Report cannot be entered yet!' + \
                                      '\n\n' + employee.name + ' has no associated Odoo User.' + \
                                      '\n\nPlease assign an Odoo User on the HR Settings tab of the Employee record and try again.')
            if employee.timesheet_cost == 0:
                raise ValidationError('This Daily Report cannot be entered yet!' + \
                                      '\n\n' + employee.name + ' has a Timesheet Cost of $0.00.' + \
                                      '\n\nPlease enter the correct Timesheet Cost on the HR Settings tab of the Employee record and try again.')

            employee_hours.append([0,0,
                {'type': 'employee',
                 'job_class_id': default_job_class_id,
    ...
    project_manager_signature = fields.Char(
        string='Approved By',
    )
